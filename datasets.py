...
from data_utils import get_normal_features, get_height_features, get_cluster_indices, batch_normals_pca, reorient_normals
...
class MultiFeatureDataset(data.Dataset):
    ...
    def __init__(self, config, shape_list_filename, seed=None, identical_epochs=False, cache_capacity=100):
        ...
        self.in_features = config.feature.in_features
        ...
        for shape_ind, shape_name in enumerate(self.shape_names):
            ...
            if not os.path.exists(nf_filename):
                ...
                # reorient normal (unoriented estimate)
                if normals_filename is not None:
                    gt_normals = np.load(normals_filename+'.npy')
                    in_normals = reorient_normals(in_normals, gt_normals)
            ...
            rot_filename = os.path.join(self.normal_dir, shape_name+'.rot.npy')
            if not os.path.exists(cidx_filename):
                ...
                np.save(rot_filename, shape_trans)
            ...
            # Get height features
            heights_filename = os.path.join(self.normal_dir, shape_name+'.heights.npy')
            if 'heights' in self.in_features and not os.path.exists(heights_filename):
                # load data
                normal_features = np.load(nf_filename)
                shape_trans = np.load(rot_filename)
                
                # compute features
                heights = get_height_features(pts, 
                                            normal_features, 
                                            shape_trans, 
                                            map_size=config.feature.map_size, 
                                            query_k=config.feature.query_k)
                np.save(heights_filename, heights)
    ...
    def __getitem__(self, index):
        ...
        # get height features
        if 'heights' in self.in_features:
            patch_heights = torch.from_numpy(shape.heights[center_point_ind, ...])
        else:
            patch_heights = None
        ...
        # other features
        if 'points' in self.in_features:
            data['points'] = patch_pts
        if 'heights' in self.in_features:
            data['heights'] = patch_heights

        # predict features
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                data[pfeat] = patch_normal
            elif pfeat == 'max_curvature':
                data[pfeat] = patch_curv[0:1]
            elif pfeat == 'min_curvature':
                data[pfeat] = patch_curv[1:2]
            else:
                raise ValueError('Unknown patch feature: %s' % (pfeat))
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        heights_filename = os.path.join(self.normal_dir, self.shape_names[shape_ind]+'.heights') if 'heights' in self.in_features else None
        ...
        if heights_filename != None:
            heights = np.load(heights_filename+'.npy')
        else:
            heights = None
        ...
        return Shape(pts=pts, nf=nf, kdtree=kdtree, normals=normals, curv=curvatures, cidx=cluster_indices, rot=rot, heights=heights)
...
class Shape():
    def __init__(self, pts, nf, kdtree, normals=None, curv=None, cidx=None, rot=None, heights=None):
        self.pts = pts
        self.nf = nf
        self.kdtree = kdtree
