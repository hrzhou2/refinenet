function [sigms normVectors errs normVectors1 neighbours_all]= compute_points_sigms_normals_two(pts, k_knn, kdtree , k_knn1)
...
normVectors1 = zeros(npts,3);
errs = zeros(npts,1);
neighbours_all = zeros(k_knn1,npts);
...
for i = 1:npts
    ...
    neighbours_all(: , i) = knn(1 : k_knn1);
    P1 = P(1 : k_knn1 , : );
    mp1 = mean(P1);
    tmp1 = P1-repmat(mp1,k_knn1,1);
    C1 = tmp1'*tmp1./k_knn1;
    if DEBUG
        [V, D] = eig(C);
        d = diag(D);
